    ...
    var mlist = {
      ...
      setReferralCode: function() {
        $.when(
          ...
          tag  = '<select id="intro" name="intro" onchange="change()">';
          for (var key in data.list) {
            tag += '<option value="'+data.url+'?qr_token='+data.qr_token+'&pr_token='+data.list[key]['pr_token']+'">'+data.list[key]['user_id'] + ':' + data.list[key]['account_name'] + '</option>';
          }
          tag += '</select>';
          $("#referral_lists").html(tag);
          $("#referral_code").html('<a href="'+$('#intro').val()+'" target="_blank">'+$('#intro').val()+'</a>');
          // $("#referral_code").html('<a href="'+data+'" target="_blank">'+data+'</a>');
        }).fail( ( data ) => {
          alert("取得失敗");
        });
      },
      setReferralChart: function() {
        $.when(
          $.ajax({
            type: 'GET',
            url: '/api/refferal_chart',
            beforeSend:function(xhr, settings){
              xhr.setRequestHeader( 'Authorization', 'Bearer '+ getStorageToken() );
            }
          })
          ...
          for (var key in data){
            data_label[i] = key;
            data_list[i] = data[key];
            i = i + 1;
          }
          createChart(data_label, data_list);
          dispChart();
        }).fail( ( data ) => {
        });
      },
...
function createChart(data_label, data) {
  ...
  var myLineChart = new Chart(ctxL, {
    ...
    data: {
      ...
      datasets: [{
        ...
        // data: [2500, 2550, 5000, 3100, 7000, 5500, 4950, 10000, 10500, 8000],
        data: data,
      }]
    },
    options: {
